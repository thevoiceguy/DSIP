...
At minimum, the protocol defines:
...
- `expires_in` is the requested subscription lifetime in seconds. Hard caps: 3,600 for `presence`, 86,400 for `publication`. Subscriptions are soft state: they expire unless renewed by a fresh `subscribe` for the same `target`+`events` (which replaces the prior subscription). `expires_in: 0` terminates a matching subscription.
...
```json
{
  ...
  "subscription": "01J5Y0QGSUB00AAAAAAAAAAAAD",
  "seq": 1,
  "state": "active",
...
| INVITING | Recv `answer` (valid) | Begin media establishment; if forked, send `cancel` (reason `session.answered-elsewhere`) | ACTIVE |
...
| PROCEEDING | Recv `answer` (valid, first) | Begin media; if forked, send `cancel` (reason `session.answered-elsewhere`) | ACTIVE |
...
| ACTIVE | Recv `bye` | Tear down media | ENDED |
| ACTIVE | Recv `cancel` | Send `error` (`session.invalid-state`); session continues | ACTIVE |
...
2. If the responder has already sent `answer` when `cancel` arrives, the responder MUST treat the session as ended and tear down any media setup in progress. It MUST NOT treat the crossed `cancel` as an error.
...
- The endpoint whose invite lost MUST send `reject` with reason `session.glare` for the losing invite and proceed as responder for the winning invite.
- The endpoint whose invite won ignores the glare condition and proceeds as initiator.
...
3. Upon accepting an answer, the initiator MUST send `cancel` with reason `session.answered-elsewhere` for the session. A forking relay MUST track which legs it delivered the invite to and which have terminated (answered, rejected, or expired), and MUST deliver the cancel **per-leg** to every leg that has not terminated. Identity-addressed fan-out without leg tracking is not conformant relay behavior: it risks re-alerting terminated legs, misses legs added mid-attempt, and cannot support targeted cancellation.
4. Any subsequent `answer` from another leg MUST be terminated by the initiator with `bye`, reason `session.already-answered`, addressed to that device.
...
4. An endpoint sending a second `update` before its first resolves is a protocol violation; the receiver responds with `error` (`session.update-pending`) and processes neither.
...
| T-Ring | Initiator | 120 s | 30–300 s | Started on first `progress` with status `ringing`. If no `answer`, cancel with reason `session.timeout`. A responder MAY extend by including `ring_timeout` (seconds) in `progress`; the initiator MAY honor it up to the upper bound. |
...
| Replay window | Both | 300 s | — | Envelopes with `issued_at` older than the window MUST be rejected. Message `id` values MUST be tracked for deduplication within the window. |
...
- `data` (required) is an object whose structure is defined by the binding named in `about`. For `transport:webrtc`, the structure above is normative in the WebRTC Media Binding document.
...
- A relay MUST NOT silently drop envelopes on a live connection. If it refuses to route an envelope (policy, rate limit, unknown recipient), it MUST respond with a signed `error` (`transport.routing-refused`, `transport.unknown-recipient`, or `transport.rate-limited`).
...
- An `answer` MUST contain the selection from that offer, and exactly one selected transport binding — an answer is a selection, not a second offer.
...
```json
{
  ...
  "session": "01J5Y0QJINT00AAAAAAAAAAAAF",
...
Glare resolution compares ULIDs, whose leading component is a timestamp under the sender's control; a malicious party can backdate its invite `id` to deterministically win glare. The direct impact is limited to role selection — who proceeds as initiator versus responder — and neither role carries billing, priority, or disclosure asymmetry in Core v1.0. Two guardrails keep it that way: (1) receivers SHOULD verify that an envelope's `id` timestamp component is consistent with its signed `issued_at` (within the replay window) and MAY reject on gross mismatch; (2) any future profile that attaches an asymmetric privilege or cost to the initiator role MUST re-evaluate glare resolution before relying on it, and MUST NOT assume role assignment is adversarially neutral.
...
A relay or transcoder does not overwrite the original publisher identity. It adds its own signed provenance statement:

```json
{
  "type": "broadcast.provenance",
  "original_stream": "did:web:wxyz.com:radio:main",
  "original_publication": "01J5Y0QEPUB00AAAAAAAAAAAAB",
...
3. Define test vectors and interop tests (JSON Schemas exist as of this draft; test vectors are the next artifact).
...
- Endpoint classes
- Integrity modes (`dsip-integrity-mode`, §22.2)
...
8. Media is established via the negotiated WebRTC binding (DTLS-SRTP). ICE candidates ride in signed `update` envelopes.
