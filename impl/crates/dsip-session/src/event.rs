...
impl LocalEvent {
    /// The session the event targets (`PlaceCall` creates it).
...
pub struct SendMsg {
    ...
    /// Session id.
    pub session: String,
    /// Message id, when chosen by the caller (updates).
...
pub enum Emission {
    ...
    Ui {
        ...
        kind: &'static str,
        /// `status` | `answered_by` | `reason` value, when the kind carries one.
    ...
    pub fn to_json(&self) -> Value {
        match self {
            Emission::Send(m) => {
                ...
                o.insert("session".into(), m.session.clone().into());
