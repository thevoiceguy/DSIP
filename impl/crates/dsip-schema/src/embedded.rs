...
//! from `v0.8/…/schemas/` at compile time; `build.rs` fails the build if they
...
macro_rules! schema {
    ($name:literal) => {
        ($name, include_str!(concat!("../../../../v0.8/dsip-schemas-v0.8-draft/dsip-schemas/schemas/", $name, ".schema.json")))
    };
}

/// `(name, json text)` for every schema in the v0.8 set (message types, the envelope and
