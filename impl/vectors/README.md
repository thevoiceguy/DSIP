...
| `blob-replicate` | `mode`, `max_blob_bytes`, `stored`, `entry {uri, sha256, size}`, `fetched? {status, sha256, size}` | `{action: fetch\|skip\|store\|discard, reason?}` (M§8.4 rule 6, spec-gap 65) |
