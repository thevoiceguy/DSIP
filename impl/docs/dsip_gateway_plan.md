...
| **G1** ◑ 2026-08-21 | `dsip-gateway` `host` feature: SIP leg (`sip_leg.rs`, real UDP + siphon-rs UAC/UAS, INVITE/ACK/BYE/CANCEL/responses with `Reason: DSIP`), DSIP leg (`dsip_leg.rs`, forge-webrtc peer connection + §12.12 candidate buffering; `Core::set_claims` added for the PSTN `tel` claim), Opus⇄G.711 bridge (`media.rs`), host loop (`call.rs`) turning leg events into `GatewayCall` steps. SIP leg proven on the wire (`tests/sip_leg_live.rs`, in CI). host is opt-in so the default build stays pure. | SIP leg live + controller-driven reject reaches the caller; DSIP-side auto-drive and a full round trip are G2 |
| **G2** ✅ 2026-08-21 | round trip proven in-process & deterministic (`tests/round_trip.rs`, in CI): a forge-media DSIP caller → gateway (real SIP on the wire) → SIP UAS peer, with Opus⇄G.711 transcoding, the real controller mediating, and the SIP peer receiving transcoded RTP; `demos/gateway-demo.sh` is the manual "vs siphond" runner. **Deviation:** CI runs the in-process test rather than building siphond (a large git dep whose call-server does SDP but not guaranteed RTP sourcing); the shell demo covers real siphond by hand. | round_trip green in CI; audio crosses the gateway both ways |
| **G3** ✅ 2026-08-21 | `dsip-core::trust` (canonical §18.1 renderer: verification basis, tel-caller headline, downgrade summary) pinned by `trust/` (13 vectors); the CLI console and the browser demo (via `dsip-wasm`) render the `tel` claim, the §18.1 basis line, and `gateway.downgraded` on inbound invites/introductions/errors; gateway `downgrade_error_detail` constructor pinned. First contact via the gateway identity: the tel claim shows on an introduction so a callee can grant the gateway. | `trust/` + `gateway/downgrade-error-*` vectors; CLI + browser wired |
| **G4** ✅ 2026-08-22 | `docs/gateway-stir-findings.md` — inbound verification available today; outbound assertion gated on operator status ((a) SP signs, (b) non-SP downgrades, (c) RFC 9060 delegate certs published in DID documents — the DSIP-shaped path); RCD>CNAM. PASSporT **signing** prototype landed in siphon-rs `sip-identity` (feature `sign`, PR #123), round-tripped through the existing verifier. | findings in `docs/`; prototype signs a PASSporT `sip-identity` verifies |
| **G5** ✅ 2026-08-21 | **Gateway Profile 1.0** (`v0.8/dsip-gateway-profile-v0.8-draft.md`) and **RTP/SRTP Media Binding 1.0** (`v0.8/dsip-rtp-srtp-media-binding-v0.8-draft.md`) drafts, transcribed from the vector-pinned `dsip-gateway` tables/controller; gateway spec-gaps 23–30 filed | both drafts in `v0.8/`; G§4/G§5/G§6/G§7/G§3 pinned by `gateway/` vectors |
