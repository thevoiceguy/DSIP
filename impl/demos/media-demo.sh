...
# (webrtc-rs). Alice sends a 440 Hz tone and records Bob; Bob sends 660 Hz and records Alice.
...
$B/dsip answer --identity "$D/bob" --relay $R --ca "$CA" --auto accept --media tone:660 --record "$D/bob-heard-alice.ogg" \
  --script "sleep 16; quit" >"$D/bob.log" 2>&1 & P=$!
sleep 1.5
echo "════════ alice calls bob with media (tone 440 Hz), talks 8 s, hangs up"
$B/dsip call --identity "$D/alice" --relay $R --ca "$CA" --to "$BOB" --media tone:440 --record "$D/alice-heard-bob.ogg" \
