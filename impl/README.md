...
| **G1/G2** gateway host + round trip — `dsip-gateway` `host` feature (opt-in): real SIP leg (siphon-rs, `Reason: DSIP`), forge-webrtc DSIP leg (§12.12 candidates), Opus⇄G.711 bridge, host loop around the G0 controller; `tests/sip_leg_live.rs` + `tests/round_trip.rs` (both in CI) prove the SIP leg on the wire and a full DSIP↔gateway↔SIP transcoded round trip; `demos/gateway-demo.sh` for real siphond | ✅ 2026-08-21 |
