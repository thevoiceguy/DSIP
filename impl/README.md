...
| speech over a DSIP call — `dsip-media` Ogg file source fixed to reassemble Opus packets across the segment table (RFC 3533) and skip OpusHead/OpusTags (was: one page = one frame, garbling multi-frame files); `demos/speech-call.sh` runs real espeak speech both ways and verifies it crossed (rhythm match 0.97+) | ✅ 2026-08-22 |
